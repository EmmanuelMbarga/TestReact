...
export const BtnCategory = ({ AllCategori,Key }) => {
  const dispatch = useDispatch();

  const handlerClicked = () => {
    dispatch(AllCategoriesAction.watchItems(AllCategori));
  };
  return (
    <>
      <div
      ...
      >
        ...
        <p className="text-sm uppercase font-inter Tablette:text-xs mobil:text-xs MiniPortable:text-xs">{AllCategori}</p>
      </div>
    </>
  );
};
